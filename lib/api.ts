...
export async function submitObservation(data: ObservationData): Promise<{ id: string; imageUrl: string }> {
  try {
    ...
    if (!observationsBucketExists) {
      ...
      const { error: createBucketError } = await supabase.storage.createBucket("observations", {
        public: true,
      })
...
export async function testSupabaseConnection() {
  try {
    ...
    // Test database
    const { data: obsCount, error: dbError } = await supabase.from("observations").select("id", { count: "exact" })
